...
import { getGalleryItems } from '@/lib/gallery/get-gallery';

export const dynamic = 'force-dynamic';

export default async function GalleryPage() {
  const memories = await getGalleryItems();
        ...
        {memories.map((memory) => (
          <Card key={memory.id} className="overflow-hidden p-0">
            {memory.kind === 'video' ? (
              ...
              <Image
                src={memory.thumbSrc ?? memory.src}
                alt={memory.title}
                width={800}
                height={1000}
                className="h-72 w-full object-cover"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    {memory.tag}
                    {memory.guestName ? ` · ${memory.guestName}` : ''}
                  </p>
                </div>
        ...
        <CardContent>
          <div className="flex flex-wrap items-center gap-3 text-sm text-muted">
            <span className="flex items-center gap-2 rounded-full bg-white px-3 py-2">
              <Camera className="h-4 w-4 text-rose" /> Photos & Videos
            </span>
            <span className="flex items-center gap-2 rounded-full bg-white px-3 py-2">
              <Sparkles className="h-4 w-4 text-rose" /> AI organization soon
            </span>
          </div>
        </CardContent>
      </Card>
